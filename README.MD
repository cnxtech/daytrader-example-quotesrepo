...
#### Create the deployment, replica set, and pod(s) to run the application
		...
		2.	$ kubectl apply -f quotes-deployment.yaml 
...
#### Create the service so clients can communicate with pods via a constant IP
        ...
        To access the service from inside the cluster,
            ...
            1.  $ kubectl get pods
                ...
                # Note the NAME of the quotes pod; you'll need it to execute the next command
            ...
            ## Notes
                ...
                a.  These 'curl' commands don't work if you run them in the quotes pod on Minikube. If you run them on another POD, they do work.
        ...
        To access the service from outside the cluster, using the API server
            ...
            2.  $ curl -k http://localhost:8001/api/v1/namespaces/default/services/https:daytrader-quotes:/proxy/health
        
                ## The above command accesses the quotes service through the API Server.
